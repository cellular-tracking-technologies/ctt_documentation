...
The station will typically receive a link-local address in the `169.254.x.x` range. This address may change between connections. Once you have your IP address you can follow the SSH instructions below.
...
Replace NetworkName with the WiFi network name (leave the quotes) and replace password with the password of the WiFi network (again, leave the quotes in the command).

Then, run:
```bash
sudo nmcli c mod CellTrackTech ipv4.method auto
```
...
### Adding WiFi Credentials Via a USB Drive {#wifi-usb}
