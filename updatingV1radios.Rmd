...
```
cd ~/Downloads (mac) or cd Downloads (pc)
...
***Note: if you run into issues resolving the host `sensorstation.local`, replace `sensorstation.local` with the IP address of your station. So, for instance, if your station IP address is `255.255.233.0` the first `scp` command above would read `scp ssr_v2_3_1.ino.hex pi@255.255.233.0:/home/pi/`.***
