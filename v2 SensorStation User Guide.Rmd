...
Yagis are directional antennas used to detect tags and nodes in a specific direction from the SensorStation. They typically have a 30-60 degree detection range that extends away from the SensorStation. For that reason typically 4 antennas are used, one pointed in each cardinal direction, or two pointed in opposite directions and used to make a “fence”. Yagis can also be used to pick up Nodes that are farther away from the SensorStation.
