...
1. All come pre-equipped with a WiFi module built into the board. This means no more worrying about WiFi dongle comparability; now it just works! 
2. There is now a dedicated Iridium port so Iridium modems can operate in tandem with LTE celluar modems if warranted.
3. There are 6 instead of 7 USB ports (one port slot was used to mount the WiFi chip mentioned above)
4. The GPS is BACK to being surface mounted to the board- so no extra GPS board like we had for the later issued V2 boards.
5. Enhanced security with new SSH login credentials. *This is super important if you are SSH'ing into your SensorStation!* 
...
CTT’s Internet of Wildlife System (IoW) is a complete radio telemetry system that consists of transmitters (radio tags), and receivers. Currently CTT produces five radio transmitters: the LifeTag^TM^, PowerTag^TM^, HybriTag^TM^, ES-200 and ES-150. 
