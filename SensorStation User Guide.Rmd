...
# json_data = plotly_json(fromJSON("changelog.json"), jsonedit = TRUE)
json_data = fromJSON("changelog.json")

for (version in c(1:nrow(json_data[1]))){
  for (feature in json_data[version, 4]) {
    for(bug in json_data[version,5]){
      
    cat('##', 'Version: ', paste0(json_data[version,1], ','), format(as.Date(json_data[version,2]), '%B %d, %Y'), '\n')
        # paste('  - Date:', json_data[version,2], '\n'),
      cat('\n')
        cat('### Patch Notes: ', json_data[version, 3], '\n')
        cat('\n')
        cat('### Features', '\n', paste('-', feature, '\n'))
        cat('\n')
        cat('### Bug Fixes', '\n', paste('-', bug, '\n'))
...
# ![](changelog.json){#id .class width=100%}
```
<!-- </details> -->
## CTT Node
...
![Option 2: LTE modem connected directly to U.FL to SMA adapter, which allows to connect directly to a Cell Booster antenna without using `J6` or `J7` on the board](Downloads/cellboost_uflToSmaConnected.JPG){#id .class width="50%"}
...
![](images/LCD_StationStats.JPG){#id .class width="20%"}
...
![](images/motus-logo.png){#id .class width=10%}
![](images/BirdsCanadaLogo.png){#id .class width=10%}
![](images/ctt_logo_300DPI.jpg){#id .class width=15%}
...
<!-- ![](images/V3PowerIn.jpeg){#id .class width=25%} -->
...
<!-- ![](images/Battery.png){#id .class width=40%} -->
...
<!-- ![](images/PI-HAT EXPANSION.png){#id .class width=50%} -->
...
<!-- ![](images/EXPANSION.png){#id .class width=50%} -->
...
<!-- ![](images/ethernetConnected.png){#id .class width=50%} -->
...
<!-- ![](images/pimoroni.cominkyphat.jpg){#id .class width=50%} -->
...
<!-- ![](images/Data Management.png){#id .class width=75%} -->
...
<!-- ![](images/am your Sensor Gnome.png){#id .class width=75%} -->
