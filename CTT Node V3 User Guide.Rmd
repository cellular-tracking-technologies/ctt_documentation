...
3. Two (2) mounting plates, one for the top of the node and one for the bottom of the node. Each plate is labeled. 
4. Solar-rechargeable power supply with 2m cable.
5. Mounting hardware
    1. Four (4) Phillips-head screws, washers, and nuts for mounting plates to node case.
    2. Three (3) bolts for mounting solar-rechargeable power supply to node plate. 
...
Your CTT Node V3 has a 16GB SD card pre-installed for backup storage, as well as for manual data collection and management.

**Hot swapping the card is not supported. It is encouraged to power down the device before inserting/removing the card.**

**If you replace the SD card with your own, prior to inserting a new SD card into a node, please ensure it has been formatted as `FAT32`**
...
The Node comes with an SD card that can be used for logging detections and other system related processes. The following features are available through the SD card.
...
*Note that you DO NOT need to format the card that came with your Node, but if you replace the SD card, you should follow the directions below to format the new card (or reformat an old one).*
