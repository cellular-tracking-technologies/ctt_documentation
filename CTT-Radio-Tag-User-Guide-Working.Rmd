...
![*BlūBird tag with 4-corner 5mm raised platform*](/Users/davidlapuma/Dropbox/CTT_Git/ctt_documentation/images/BlūBirdMagnetOff.jpg){#id .class width=50%}
...
![*BlūBird with 4-corner hole platform as shipped with magnet attached*](/Users/davidlapuma/Dropbox/CTT_Git/ctt_documentation/images/BlūBirdMagnetOn.jpg){#id .class width=50%}
...
BlūBird tags are shipped in a low-power state and may not arrive fully charged. Before deployment, all tags should be checked and charged to ensure proper operation in the field.

### Activate or Test the Tag

Using your Sidekick receiver:

1. Remove the magnet from one BlūBird tag.
2. Watch the Sidekick display for the tag to appear.
3. Confirm that the tag is transmitting and reporting battery voltage.
4. Repeat for each tag in the shipment.

### Confirm Detection

Confirm that each tag appears on the Sidekick display, is transmitting normally, and is reporting battery voltage.

<!-- Confirm whether the guide should also include SensorStation / BlūSeries Receiver / Motus / Blū+ portal detection steps for BlūBird, or keep this section limited to pre-deployment Sidekick verification. -->

### Charging / Battery Notes

As each tag appears on the Sidekick display, note the reported battery voltage.
...
Because BlūBird tags are not shipped fully charged, it is normal for some tags to require charging before use. We highly recommend charging all tags prior to deployment.

Recommended charging procedure:

1. Place the tags outdoors in direct sunlight whenever possible.
2. Allow the tags to charge for at least 6-12 hours.
3. For best results, charge the tags for 1-2 full days before deployment.

The magnets may remain attached during charging if desired, although this is not required for charging to occur.
...
If the tag still does not appear, charge the tag and then check it again.
...
- ☐ Battery voltage approximately 3.0 V or higher.
- ☐ Tag charged in sunlight for at least 6-12 hours; 1-2 days recommended.
- ☐ Final operation verified before attachment to bird.
