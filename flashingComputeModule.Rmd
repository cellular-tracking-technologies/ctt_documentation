...
Click [here](https://github.com/cellular-tracking-technologies/sensor-station-software/releases) to go to the image release and download page. You will also find information on releases and updates.

### **Old and Depreciated Disk Images**

These disk images are no longer updated with the latest software.
...
[**Version ID: 1.7.0 Image: 2025-09-17 Updated: 2025-11-24**](https://s3.amazonaws.com/media.celltracktech.com/sensor-station/images/sensor-station.latest.xz) **Replaced by `Version 2.3.4` linked above. This image is compatible with the CTT Node V3 and the BluSeries Receiver.**
...
[**Version ID: 1.3.0 Image 2023-05-01 Updated 2023-05-01**](https://s3.amazonaws.com/media.celltracktech.com/sensor-station/images/sensor-station.2023-05-01.zip) **Replaced by `Version 1.7.0` linked above** 

**Default radio firmware for this older LTS image is v3.0.1 and is not compatible with CTT Node V3 or the BluSeries Receiver**.
...
![](images/rpi-imager-2.x.x/select-device.png){#id .class width=50%}

9.  for `Raspberry Pi Device` you can just leave it as-is. You do not need to select anything.
10. Click the button under `Operating System` and scroll down to the very bottom to `Use Custom`.

![](images/rpi-imager-2.x.x/select-disk-image.png){#id .class width=50%}

Click on it and choose to the SensorStation image file you saved (it's a `.zip` or `.xz` file), and select it.

![](images/rpi-imager-2.x.x/select-disk-image-file.png){#id .class width=50%}

11. Click the button under `Storage` and select your Raspberry Pi compute module as the target. `example: RPi-MSGD- 0001 (bootfs, rootfs), 14.6 GB`

![](images/rpi-imager-2.x.x/select-storage-device.png){#id .class width=50%}

12. Click the `NEXT` button. You will get a warning message saying you are about to erase all data on the RPi Module.

![](images/rpi-imager-2.x.x/warning-message.png){#id .class width=50%}


**IF YOU HAVEN'T ALREADY, DOWNLOAD YOUR DATA BEFORE PROCEEDING TO THE NEXT STEP!!!**

13. Click "I UNDERSTAND, ERASE AND WRITE". You will then get this screen to monitor the disk writing completion:

![](images/rpi-imager-2.x.x/writing-0-percent.png){#id .class width=50%}


  *   This will take a while, but once the image is written you will get a success message and the process is complete. Select Finish.
  
![](images/rpi-imager-2.x.x/write-complete.png){#id .class width=50%}
